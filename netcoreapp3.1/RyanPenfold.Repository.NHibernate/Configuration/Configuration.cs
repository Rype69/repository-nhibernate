    ...
    public class Configuration : IConfiguration
    {
        ...
        public static IConfigurationSection ConfigurationSection
        {
            get
            {
                ...
                var possibleConfigurationSectionNames = new[] { assemblyName, "nhibernateRepository", "Repository.NHibernate" };

                var count = 0;
                do
                {
                    _configurationSection = RyanPenfold.Configuration.SettingsProvider<Json.ConfigurationSection>.GetSection(possibleConfigurationSectionNames[count]);
                    count++;
                } while (_configurationSection == null && count < possibleConfigurationSectionNames.Length);
        ...
        public global::NHibernate.ISessionFactory BuildSessionFactory()
        {
            ...
            var connectionString = RyanPenfold.Configuration.ConnectionStrings.Get(ConfigurationSection.ConnectionStringName);
            ...
            foreach (Mapping mappingConfigElement in ConfigurationSection.GetMappings())
            {
                ...
                if (!string.IsNullOrWhiteSpace(mappingConfigElement.Type) && !Enum.TryParse(mappingConfigElement.Type, out type))
                {
                    throw new FormatException($"Invalid mapping type \"{mappingConfigElement.Type}\".");
                }

                // Attempt to load the specified mapping
                var assembly = Assembly.Load(mappingConfigElement.Assembly);

                //// If the assembly is not loaded, throw an exception
                //if (assembly == null)
                //{
                //    throw new Exception($"Assembly {mappingConfigElement.Assembly} not found");
                //}
