...
namespace RyanPenfold.Repository.NHibernate
{
    ...
    public class BaseClassMap<T> : ClassMap<T>, IClassMap
    {
        ...
        public void EnsureColumnExists(string tableSchema, string tableName, string columnName, string columnType)
        {
            ...
            var columnExists = Convert.ToBoolean(RyanPenfold.Utilities.Data.SqlClient.SqlCommand.RunExecuteScalar(
                ...
                connectionStringName: Configuration.Configuration.ConfigurationSection.ConnectionStringName));

            if (!columnExists)
            {
                RyanPenfold.Utilities.Data.SqlClient.SqlCommand.RunExecuteNonQuery(
        ...
        public void EnsureTableExists(Type mappedEntityType, string tableSchema, string tableName)
        {
            ...
            if (!tableExists)
            {
                // TODO: In order to avoid sql_variant typed columns, determine which properties are of DTO type and therefore reference foreign keys
